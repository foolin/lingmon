...
public partial class User_ReSendActiveEmail : System.Web.UI.Page
{
    ...
    protected void btnSubmit_Click(object sender, EventArgs e)
    {
        ...
        if (user.Status != 0)
        {
            WebAgent.AlertAndBack("您的帐号无需重新激活！");
            return;
        }
        ...
        try
        {
            ...
            string mailTitle = mailTpl.GetRegisterTitle(user.UserName);
            string mailContent = mailTpl.GetRegisterInfo(user);
            mail.Send(user.Email, mailTitle, mailContent, true);
        }
        catch (Exception ex)
        {
            WebLog.WriteErrLog("发送找回密码异常，用户输入：" + strFindValue + "，异常：" + ex.Message);
            WebAgent.AlertAndBack("发送邮件失败，请稍后再试或者联系管理员！");
            return;
        }

        if (isEmail)
        {
            WebAgent.AlertAndGo("已经重发激活邮件到注册邮箱" + strFindValue + "，请及时激活！", "../Default.aspx");
        }
        else
        {
            WebAgent.AlertAndGo("已经重发激活邮件到注册邮箱" + GetHideEmail(user.Email) + "，请及时重置激活！", "../Default.aspx");
        }
