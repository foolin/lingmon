...
public partial class Manage_Article_ArtList : AdminBase
{
    string type = "";
    ...
    public string GetStatusDesc(object status, object artid)
    {
        ...
        switch (iStatus)
        {
            case -1:
                strStatus = " <u><font color='gray'>已删除</font></u> |  "
                    + " <a href='ArtSetStatus.aspx?id=" + artid + "&status=0'>还原为未审核</a> | "
                    + " <a href='ArtSetStatus.aspx?id=" + artid + "&status=1'>还原为已审核</a> ";
                break;
            case 0:
                strStatus = " <font color='red'>未审核</font> |  "
                    + " <a href='ArtSetStatus.aspx?id=" + artid + "&status=1'>通过审核</a> | "
                    + " <a href='ArtSetStatus.aspx?id=" + artid + "&status=-1' onclick='return confirm(\"确定删除？\")'>删除</a> ";
                break;
            case 1:
                strStatus = " <font color='#090'>已审核 √</font> |  "
                    + " <a href='ArtSetStatus.aspx?id=" + artid + "&status=0'>取消审核</a> | "
                    + " <a href='ArtSetStatus.aspx?id=" + artid + "&status=-1' onclick='return confirm(\"确定删除？\")'>删除</a> ";
                break;
        }
